...
export default function StatusCard({ complaint }) {
  ...
  return (
    <div className={`card p-6 hover:shadow-xl transition-all duration-300 ${config.border} overflow-hidden group`}>
      ...
      {/* Details Grid */}
      <div className="grid grid-cols-2 gap-4 mb-6 pb-6 border-t border-gray-200">
        <div>
        ...
        <div>
          <p className="text-xs text-gray-600 font-bold uppercase tracking-widest mb-1">Filed On</p>
          <p className="text-sm font-bold text-gray-900">{new Date(complaint.created_at).toLocaleDateString()}</p>
        </div>
      </div>

      {/* Action Button */}
      <button className="w-full bg-blue-50 hover:bg-blue-100 text-blue-900 py-3 rounded-lg font-bold transition-all duration-200 flex items-center justify-center gap-2">
